...
/** Downsamples a (potentially thousand-point) parsed track to at most `maxPoints` lat/lng pairs — plenty to preserve the route's visual shape when plotted small on the intro card in `video-engine.ts`, without shipping every raw GPS fix into the render pipeline. */
export function downsampleRoutePoints(points: GpxTrackPoint[], maxPoints = 150): { lat: number; lng: number }[] {
  if (points.length === 0) return [];
  if (points.length <= maxPoints) return points.map((p) => ({ lat: p.lat, lng: p.lng }));
...
/** Parses raw GPX XML directly in the browser so the 3D preview can run without depending on Leaflet rendering. */
export function parseTrackPointsFromGpxText(gpxText: string): GpxTrackPoint[] {
  if (typeof DOMParser === "undefined") return [];
  const xml = new DOMParser().parseFromString(gpxText, "application/xml");
  if (xml.querySelector("parsererror")) return [];

  const pointNodes = Array.from(xml.querySelectorAll("trkpt, rtept"));
  return pointNodes
    .map((node) => {
      const lat = Number(node.getAttribute("lat"));
      const lng = Number(node.getAttribute("lon") ?? node.getAttribute("lng"));
      ...
      const parsedTime = timeText ? new Date(timeText) : null;
      return {
        lat,
        lng,

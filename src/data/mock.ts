...
import { buildEmotionalStoryTimeline } from "@/lib/emotional-story-curve-engine";
...
export function generateMockAnalysis(
  ...
  return {
    ...
    bestMoments,
    emotionalStory: buildEmotionalStoryTimeline(null, style, gpxStats ?? null, { bestMoments, durationSeconds: totalDuration }),
    hooks,
    captions,
...
export function generateMockAnalysisMulti(
  ...
  return buildAnalysisResult(videoName, totalDuration, style, bestMoments, gpxStats ?? null);
}
...
export function buildAnalysisResult(
  ...
  // Overall "virality" score is derived deterministically from the real moment
  // confidences (plain average, clamped to a sane display range) so the same
  // footage always reproduces the same score instead of jittering a few
  // points on every re-render — a random score wasn't just cosmetic noise, it
  // undermined the "this reflects genuine analysis" promise of the feature.
  const avgConfidence = bestMoments.length
    ? bestMoments.reduce((sum, m) => sum + m.confidence, 0) / bestMoments.length
    : 78;
  const overallScore = Math.round(Math.min(99, Math.max(55, avgConfidence)));
  ...
  return {
    ...
    overallScore,
    bestMoments,
    emotionalStory: buildEmotionalStoryTimeline(null, style, gpxStats ?? null, { bestMoments, durationSeconds: totalDurationSeconds }),
    hooks,
    captions,
    hashtags: HASHTAGS_BY_STYLE[style],
    music: MUSIC_BY_STYLE[style],

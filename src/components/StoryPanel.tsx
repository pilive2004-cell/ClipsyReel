...
import { BestMoment, MontageResult, ReelStyle, UploadedVideo } from "@/types";
...
export default function StoryPanel({ videos, style, bestMoments, watermark }: StoryPanelProps) {
  ...
  const handleDownload = async () => {
    if (!result) return;
    const filename = `clipsyreel-${videos[0]?.name.replace(/\.[^.]+$/, "") ?? "reel"}-story.mp4`;
    await saveVideoToDevice(result.blob, filename);
  };

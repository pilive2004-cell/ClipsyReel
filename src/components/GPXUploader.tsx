...
import { GpxRouteStats, GpxTrackPoint, UploadedVideo, VideoLocationMatch } from "@/types";
import RouteStatsCard from "./gpx/RouteStatsCard";
import VideoLocationMatcher from "./gpx/VideoLocationMatcher";
import VideoMetadataScanner from "./gpx/VideoMetadataScanner";
...
export default function GPXUploader({
  ...
  /** Rich location matches from the Smart Video Location Matcher, kept in state so the map can show them. */
  const [locationMatches, setLocationMatches] = useState<VideoLocationMatch[]>([]);
  ...
  if (gpxText) {
    return (
      <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4 space-y-3">
        ...
        {stats && <RouteStatsCard stats={stats} />}

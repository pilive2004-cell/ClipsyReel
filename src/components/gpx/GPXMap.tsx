...
import { GpxRouteStats, GpxTrackPoint, VideoLocationMatch, VideoRouteMatch } from "@/types";
import { computeRouteStatsFromPoints, extractTrackPoints, parseTrackPointsFromGpxText } from "@/lib/gpx";
...
interface GPXMapProps {
  ...
  /** Rich matches from the Smart Location Matcher (Level 1/2 automatic + Level 3 manual). */
...
/** Adds the real parsed GPX polyline to the map (via `leaflet-gpx`), reports back extracted points/stats, and fits a calmer TravelBoast-style overview once loaded. */
function GpxRouteLayer({
  ...
  useEffect(() => {
    ...
    layer.on("loaded", () => {
      ...
      map.flyToBounds(bounds, { padding: [48, 48], duration: 1.1, maxZoom: 11 });
...
/**
 * Real, interactive Leaflet route map for Pro users.
 * Renders the parsed GPX track, start/end markers, and one marker per
 * uploaded video that could be matched to a route point.
 */
export default function GPXMap({ gpxText, videoMatches, locationMatches, onReady, heightClassName = "h-64" }: GPXMapProps) {
  const icons = useMapIcons();
  const [loadedForTrack, setLoadedForTrack] = useState<string | null>(null);
  const points = useMemo(() => parseTrackPointsFromGpxText(gpxText), [gpxText]);
  const stats = useMemo(() => (points.length > 0 ? computeRouteStatsFromPoints(points) : null), [points]);
  const first = points[0] ?? null;
  const last = points[points.length - 1] ?? null;

  useEffect(() => {
    if (!stats || points.length === 0) return;
    onReady?.(points, stats);
  }, [onReady, points, stats]);

  const handleLoaded = (loadedPoints: GpxTrackPoint[], loadedStats: GpxRouteStats) => {
    setLoadedForTrack(gpxText);
    onReady?.(loadedPoints, loadedStats);
  };

  const mapLoaded = loadedForTrack === gpxText;
    ...
    <div className={`relative w-full overflow-hidden rounded-xl border border-white/10 ${heightClassName}`}>
      ...
      {icons && (
        ...
        >
          ...
          {points && <VideoMarkerLayer matches={videoMatches} locationMatches={locationMatches} iconFor={(status) => icons.video("gps")} />}
        </MapContainer>
      )}
    </div>
  );
